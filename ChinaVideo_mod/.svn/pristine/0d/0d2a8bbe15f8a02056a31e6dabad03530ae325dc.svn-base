		...
		if(url!=""){
			if(url.equals("upload")){
				...
				imageView.setImageResource(R.drawable.img_uploadphoto);
				 ...
				 if(position < GlobalData.getInstance(mContext).mPhotoAblumImgPath.size()){
					 imageView.setTag(url);
	...
	public void setLocation(int location) {
		if(location == uploadIndex){
			...
			if(!mVecSelect.contains(location)){
				...
				mVecSelectPhotoPath.add(gd.mPhotoAblumImgPath.elementAt(location)) ;
			}else{
				mVecSelect.removeElement(location);
				mVecSelectPhotoPath.removeElement(gd.mPhotoAblumImgPath.elementAt(location));
			}
		}
...
//		notifyDataSetChanged();
	...
	public void doDelete() {
		...
		while(mVecSelectPhotoPath.size()>0){
			...
			GlobalData gd = GlobalData.getInstance(mContext);
			if(gd.mPhotoAblumImgPath.contains(photo)){
				((MyPhotoAlbumActivity)mContext).callDeletePhoto(photo.id) ;
				gd.mPhotoAblumImgPath.remove(photo);
			}
