...
public class FirstPayActivity extends Activity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		gd = GlobalData.getInstance(FirstPayActivity.this);
		...
		button_350.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (gd.checkLogin()) {
					RpcRoutine.getInstance().addRpc(RpcEvent.GetShouChongGift, handler,FirstPayActivity.this,
							gd.getSharedPreferences().getString(APP.USER_ID, ""),
							gd.getSharedPreferences().getString(APP.SECRET, ""), 1);
				}
				else {
					gd.mGetShouchongTips = "您账号尚未登录，请先登陆";
					showTips(Type_Login);
				}
		...
		button_1000.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (gd.checkLogin()) {
					...
					RpcRoutine.getInstance().addRpc(RpcEvent.GetShouChongGift, handler,FirstPayActivity.this,
							gd.getSharedPreferences().getString(APP.USER_ID, ""),
							gd.getSharedPreferences().getString(APP.SECRET, ""), 2);
				}
				else {
					gd.mGetShouchongTips = "您账号尚未登录，请先登陆";
					showTips(Type_Login);
	...
	private void showTips(final int type) {
		...
		textshow.setText(gd.mGetShouchongTips);
