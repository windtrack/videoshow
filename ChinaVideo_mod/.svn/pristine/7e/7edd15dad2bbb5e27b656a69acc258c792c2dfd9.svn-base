...
//		m_anchorNewsView.setAdapter(adapter);
	}
	/**
	 * 显示主播动态
	 */
...
//			m_AnchorNewAdapter = new AnchorNewAdapter(this, 0, GlobalData.getInstance().mAnchorNewsInfo) ;
...
//				m_AnchorNewAdapter.notifyDataSetChanged();
			...
			case HandlerCmd.HandlerCmd_GetAnchorNewsFailed:
			{
				...
				String headUrl = APP.POST_URL_ROOT + anchor_icon ;
				
				GlobalData.getInstance().mAnchorNewsInfo.clear();
				...
				GlobalData.getInstance().mAnchorNewsInfo.add(oneNews);
...
//			}
			...
			public void run() {
				try {
					String user_id = sharedPreferences.getString(APP.USER_ID,
							"");
					String secret = sharedPreferences.getString(APP.SECRET, "");
					String result = Utils.addRpcEvent(RpcEvent.GetPhotoWall,user_id, secret,anchor_id, 1,5);
					JSONObject obj = new JSONObject(result);
					int s = obj.getInt("s");
					GlobalData.getInstance().mAnchorNewsInfo.clear();
					if (s == 1) {
	...
	private boolean checkIsShowing(){
		...
		if(mMediaPlayer==null || !mMediaPlayer.isPlay()){
			return false;
		}
		return true ;
	}
