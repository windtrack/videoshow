...
public class LoginActivity extends Activity implements OnClickListener {
	...
	private void login(final String name, final String openid,
		...
		if ("wechat_login".equals(name)) {
			RpcRoutine.getInstance().addRpc(RpcEvent.CallWXLogin, handler,  openid, token,50,deviceToken, wx_qq_channel);
		} else if ("m_qq_login".equals(name)) {
			RpcRoutine.getInstance().addRpc(RpcEvent.CallQQLogin, handler,  openid, token,50,deviceToken, wx_qq_channel);
		} else if ("m_user_login".equals(name)){
			RpcRoutine.getInstance().addRpc(RpcEvent.CallUserLogin, handler,  openid, token, 50,deviceToken);
		}
	...
	private Handler handler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			...
			case HandlerCmd.HandlerCmd_CallLoginException:
				...
				data =GlobalData.getInstance().mLoginErrorInfo;
