...
public class PersonalCenterFragment extends Fragment implements OnClickListener {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		if (rootView == null) {
			...
			if(GlobalData.getInstance(getActivity()).isAnchor()){
	...
	public void onClick(View v) {
		...
		case R.id.rl_photoalbum:
			...
			if(GlobalData.getInstance(getActivity()).checkLogin()){
				...
				in.putExtra("anchorId", GlobalData.getInstance(getActivity()).getSharedPreferences().getString(APP.USER_ID, ""));
				startActivity(in);
			}else{
				callLogin() ;
			}
	...
	private Handler handler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			...
			case HandlerCmd.HandlerCmd_Call3rdLoginSuccess:
				...
				if (GlobalData.getInstance(getActivity()).getmThirdLoginInfo().username != ""
						&& GlobalData.getInstance(getActivity()).getmThirdLoginInfo().userToken != "") {
					String deviceToken = GlobalData.getInstance(getActivity())
							.getSharedPreferences()
							.getString(APP.DEVICETOKEN, "");
					RpcRoutine.getInstance()
							.addRpc(RpcEvent.Call3rdLogin,
									handler,getActivity(),
									GlobalData.getInstance(getActivity())
